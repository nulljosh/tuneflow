...
function drawCurvedText(text, centerX, centerY, radius, startAngle, fontSize) {
    ctx.font = `bold ${fontSize}px Impact, Arial Black, sans-serif`;
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'center';
    
    const angleStep = 0.3; // Adjust for curve tightness
...
// Draw "HAPPY BIRTHDAY" curved at top (arc upward)
drawCurvedText('HAPPY BIRTHDAY', width / 2, 250, 750, -Math.PI * 0.5, 95);

// Draw "GRAMMA!" straight across middle
ctx.font = 'bold 180px Impact, Arial Black, sans-serif';
ctx.textBaseline = 'middle';
const grammaText = 'GRAMMA!';
const totalWidth = ctx.measureText(grammaText).width;
let xPos = (width - totalWidth) / 2;
const yPos = height / 2 + 80;

grammaText.split('').forEach((char, i) => {
    const color = colors[i % colors.length];
...
console.log('Image saved to: /Users/joshua/.openclaw/workspace/80s-birthday-gramma.png');
